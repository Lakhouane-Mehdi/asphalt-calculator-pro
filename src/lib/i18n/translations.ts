export type Language = 'en' | 'de';

export const translations = {
    en: {
        title: "Quick Estimate",
        subtitle: "Standard Volume & Tonnage",
        projectName: "Project Name",
        clientName: "Client Name",
        length: "Length",
        width: "Width",
        thickness: "Thickness",
        density: "Density",
        frostZone: "Frost Zone",
        compaction: "Thickness Type",
        compacted: "Compacted (Finished)",
        loose: "Loose (Screed)",
        estLoose: "Est. Loose Laydown",
        estCompacted: "Est. Compacted",
        materialType: "Material Type",
        materialCategory: "Material Category",
        catAsphalt: "Asphalt",
        catConcrete: "Concrete",
        catPaving: "Paving Stones",
        catBase: "Base Layer (Gravel)",
        pricePerTon: "Price / Ton",
        calculateCost: "Calculate Cost",
        totalEstimate: "Total Estimate",
        requiredTonnage: "Required Tonnage",
        estCoverage: "Est. Coverage",
        exportQuote: "Export Quote",
        exportQuotePdf: "Export Quote PDF",
        exportInvoicePdf: "Export Invoice PDF",
        settings: "Settings",
        language: "Language",
        newProject: "New Project",
        newProjectConfirm: "Clear all project data and start a new quote?",
        currency: "Currency",
        wastePercent: "Waste Allowance (%)",
        wasteHint: "Extra material for spillage, cold joints and truck residue",
        netTonnage: "Theoretical",
        withWaste: "incl. waste",
        documentType: "Document Type",
        docQuote: "Quote",
        docInvoice: "Invoice",
        companyProfile: "Company Details",
        companyHint: "Required on invoices. Stored only on this device.",
        companyName: "Company Name",
        companyAddress: "Address",
        companyTaxId: "Tax ID / VAT ID",
        companyContact: "Phone / Email",
        paymentTerms: "Payment Terms",
        invoicePrefix: "Invoice Prefix",
        nextInvoiceNumber: "Next Invoice No.",
        invoiceMissingDetails: "Add your company name, address and tax ID to issue a compliant invoice.",
        layerNames: {
            surface: "Surface Course",
            binder: "Binder Course",
            base: "Base Course",
            generic: "Layer"
        },
        jobs: {
            title: "Saved Jobs",
            subtitle: "Your project history",
            save: "Save Job",
            saved: "Saved",
            update: "Update Job",
            open: "Open",
            duplicate: "Duplicate",
            delete: "Delete",
            deleteConfirm: "Delete this job permanently?",
            empty: "No saved jobs yet. Save a job to find it here later.",
            emptyHint: "Everything is stored on this device only.",
            copySuffix: "(copy)",
            unnamed: "Unnamed project",
            noClient: "No client",
            created: "Created",
            updated: "Updated",
            exportAll: "Export All",
            importFile: "Import",
            importDone: "Imported {{n}} job(s)",
            importFailed: "Could not read that file",
            nameRequired: "Give the project a name before saving.",
            unsavedChanges: "Unsaved changes",
            openJob: "Open saved job",
            search: "Search jobs…"
        },
        madeBy: "Made by Mehdi Lakhouane",
        madeByShort: "by Mehdi L.",
        placeholders: {
            project: "e.g. Main Street Overlay",
            client: "e.g. City Council",
            length: "0.0",
            width: "0.0",
            thickness: "0.0",
            density: "2.4",
            price: "Price / Ton",
            tonnage: "e.g. 500",
            plantRate: "e.g. 150",
            truckInfo: "e.g. 20",
            cycleTime: "e.g. 60",
            rap: "e.g. 20"
        },
        logistics: {
            title: "Logistics Planner",
            subtitle: "Fleet Optimization",
            plantRate: "Plant Rate (TPH)",
            truckCapacity: "Truck Capacity (t)",
            cycleTime: "Cycle Time (min)",
            fleetRequired: "Fleet Required",
            trucks: "Trucks",
            matchSpeed: "To match plant speed",
            spacing: "Spacing Interval",
            mins: "mins",
            betweenArrivals: "Between arrivals"
        },
        sustainability: {
            title: "Green Asphalt",
            subtitle: "Sustainability Metrics",
            totalTonnage: "Total Tonnage (t)",
            rapPercent: "RAP Percentage (%)",
            co2: "CO2 Footprint",
            emissions: "Est. Emissions",
            binderSaved: "Virgin Binder Saved",
            basedOn: "Based on",
            basedOnRap: "Based on {{percent}}% RAP"
        },
        cooling: {
            title: "Cooling Predictor",
            subtitle: "Time until cessation (80°C)",
            mixTemp: "Mix Temp (°C)",
            airTemp: "Air Temp (°C)",
            windSpeed: "Wind (km/h)",
            thickness: "Lift Thickness (cm)",
            timeAvailable: "Time Available",
            coreTempNote: "Until core temp < 80°C",
            warning: "Warning: Rapid compaction required!"
        },
        vision: {
            title: "Smart Measurement",
            subtitle: "Photo-to-Area (Beta)",
            startCamera: "Start Camera",
            permission: "Camera permission required to measure area.",
            alignCard: "Align Credit Card Here",
            tapCorners: "Tap corner points of damage",
            retake: "Retake",
            measuredArea: "Measured Area",
            calculate: "Calculate Area"
        },
        ar: {
            title: "AR Visualizer",
            subtitle: "Virtual Grid Overlay",
            view: "View AR",
            close: "Close AR",
            gridLabel: "Grid: ~0.5m Tiles",
            tilt: "Tilt (Perspective)",
            height: "Height",
            instruction: "Adjust Tilt & Height to align the grid with the floor."
        },
        units: {
            length: "m",
            thickness: "cm",
            density: "t/m³",
            tonnage: "t",
            area: "m²"
        },
        step: "Step",
        of: "of",
        back: "Back",
        next: "Next",
        projectDetails: "Project",
        dimensions: "Dimensions",
        material: "Material",
        summary: "Summary",
        calculationsVerified: "Calculations Verified",
        trafficClass: "Traffic Class (RStO 12)",
        recommendedStructure: "Recommended Structure",
        thicknessWarning: "Note: RStO recommendation overrides current values",
        none: "None",
        privacyTitle: "Privacy in Focus",
        privacyDescription: "We value your privacy. We use cookies to improve your experience and analyze our traffic. In accordance with German TDDDG, we only track anonymous country data after your explicit consent.",
        custom: "Custom",
        rejectAll: "Reject All",
        acceptAll: "Accept All",
        settingsTitle: "Cookie Settings",
        essentialCookies: "Essential Cookies",
        essentialCookiesDesc: "Always active. Needed for the app to function correctly.",
        analyticsCookies: "Analytics (Anonymous)",
        analyticsCookiesDesc: "Helps us understand from which countries our users come from.",
        apply: "Apply Selection",
        downloadReport: "Download PDF Report",
        modeWorker: "Worker Mode",
        modeEngineer: "Engineer Mode",
        modeWorkerDesc: "Quick calculation for tonnage",
        modeEngineerDesc: "RStO 12 standards & advanced structures"
    },
    de: {
        title: "Schnellkalkulation",
        subtitle: "Standard Volumen & Tonnage",
        projectName: "Projektname",
        clientName: "Kundenname",
        length: "Länge",
        width: "Breite",
        thickness: "Schichtdicke",
        density: "Dichte",
        frostZone: "Frostzone (RStO)",
        compaction: "Dickenart",
        compacted: "Verdichtet (Fertig)",
        loose: "Lose (Einbau)",
        estLoose: "Einbaustärke (Lose)",
        estCompacted: "Endstärke (Verdichtet)",
        materialType: "Material Type",
        materialCategory: "Material Category",
        catAsphalt: "Asphalt",
        catConcrete: "Concrete (Beton)",
        catPaving: "Paving Stones (Pflaster)",
        catBase: "Base Layer (Tragschicht)",
        pricePerTon: "Price / Ton",
        calculateCost: "Kosten berechnen",
        totalEstimate: "Gesamtschätzung",
        requiredTonnage: "Benötigte Menge",
        estCoverage: "Deckfläche",
        exportQuote: "Angebot exportieren",
        exportQuotePdf: "Angebot als PDF",
        exportInvoicePdf: "Rechnung als PDF",
        settings: "Einstellungen",
        language: "Sprache",
        newProject: "Neues Projekt",
        newProjectConfirm: "Alle Projektdaten löschen und neues Angebot beginnen?",
        currency: "Währung",
        wastePercent: "Verschnittzuschlag (%)",
        wasteHint: "Zusatzmaterial für Verschnitt, Nähte und Muldenreste",
        netTonnage: "Theoretisch",
        withWaste: "inkl. Verschnitt",
        documentType: "Dokumentart",
        docQuote: "Angebot",
        docInvoice: "Rechnung",
        companyProfile: "Firmendaten",
        companyHint: "Für Rechnungen erforderlich. Nur auf diesem Gerät gespeichert.",
        companyName: "Firmenname",
        companyAddress: "Anschrift",
        companyTaxId: "Steuernr./USt-IdNr.",
        companyContact: "Telefon / E-Mail",
        paymentTerms: "Zahlungsbedingungen",
        invoicePrefix: "Rechnungspräfix",
        nextInvoiceNumber: "Nächste Rechnungsnr.",
        invoiceMissingDetails: "Firmenname, Anschrift und Steuernummer angeben, um eine gültige Rechnung zu erstellen.",
        layerNames: {
            surface: "Deckschicht",
            binder: "Binderschicht",
            base: "Tragschicht",
            generic: "Schicht"
        },
        jobs: {
            title: "Gespeicherte Projekte",
            subtitle: "Ihr Projektverlauf",
            save: "Projekt speichern",
            saved: "Gespeichert",
            update: "Projekt aktualisieren",
            open: "Öffnen",
            duplicate: "Duplizieren",
            delete: "Löschen",
            deleteConfirm: "Dieses Projekt endgültig löschen?",
            empty: "Noch keine gespeicherten Projekte. Speichern Sie ein Projekt, um es hier wiederzufinden.",
            emptyHint: "Alles wird nur auf diesem Gerät gespeichert.",
            copySuffix: "(Kopie)",
            unnamed: "Unbenanntes Projekt",
            noClient: "Kein Kunde",
            created: "Erstellt",
            updated: "Geändert",
            exportAll: "Alle exportieren",
            importFile: "Importieren",
            importDone: "{{n}} Projekt(e) importiert",
            importFailed: "Datei konnte nicht gelesen werden",
            nameRequired: "Bitte zuerst einen Projektnamen vergeben.",
            unsavedChanges: "Nicht gespeichert",
            openJob: "Gespeichertes Projekt öffnen",
            search: "Projekte suchen…"
        },
        madeBy: "Entwickelt von Mehdi Lakhouane",
        madeByShort: "von Mehdi L.",
        placeholders: {
            project: "z.B. Hauptstraße Sanierung",
            client: "z.B. Stadtverwaltung",
            length: "0,0",
            width: "0,0",
            thickness: "0,0",
            density: "2,4",
            price: "Preis / Tonne",
            tonnage: "z.B. 500",
            plantRate: "z.B. 150",
            truckInfo: "z.B. 20",
            cycleTime: "z.B. 60",
            rap: "z.B. 20"
        },
        logistics: {
            title: "Logistikplaner",
            subtitle: "Flottenoptimierung",
            plantRate: "Mischanlagenleistung (t/h)",
            truckCapacity: "LKW-Kapazität (t)",
            cycleTime: "Umlaufzeit (min)",
            fleetRequired: "Benötigte Flotte",
            trucks: "LKWs",
            matchSpeed: "Zur Anlagenauslastung",
            spacing: "Taktzeit",
            mins: "min",
            betweenArrivals: "Zwischen Ankünften"
        },
        sustainability: {
            title: "Grüner Asphalt",
            subtitle: "Nachhaltigkeitskennzahlen",
            totalTonnage: "Gesamtmenge (t)",
            rapPercent: "RAP Anteil (%)",
            co2: "CO2 Fußabdruck",
            emissions: "Geschätzte Emissionen",
            binderSaved: "Eingespartes Bitumen",
            basedOn: "Basierend auf",
            basedOnRap: "Basiert auf {{percent}}% Ausbauasphalt"
        },
        cooling: {
            title: "Einbaufenster",
            subtitle: "Zeit bis Verdichtungsende (80°C)",
            mixTemp: "Mischgut (°C)",
            airTemp: "Luft (°C)",
            windSpeed: "Wind (km/h)",
            thickness: "Einbaudicke (cm)",
            timeAvailable: "Verfügbare Zeit",
            coreTempNote: "Bis Kerntemperatur < 80°C",
            warning: "Achtung: Schnelle Verdichtung nötig!"
        },
        vision: {
            title: "Smart Aufmaß",
            subtitle: "Foto-zu-Fläche (Beta)",
            startCamera: "Kamera starten",
            permission: "Kamerazugriff erforderlich.",
            alignCard: "Kreditkarte hier ausrichten",
            tapCorners: "Eckpunkte des Schadens antippen",
            retake: "Wiederholen",
            measuredArea: "Gemessene Fläche",
            calculate: "Fläche berechnen"
        },
        ar: {
            title: "AR Visualisierung",
            subtitle: "Virtuelles Gitter-Overlay",
            view: "AR ansehen",
            close: "AR schließen",
            gridLabel: "Gitter: ~0,5m Kacheln",
            tilt: "Neigung",
            height: "Höhe",
            instruction: "Neigung & Höhe anpassen um Gitter am Boden auszurichten."
        },
        units: {
            length: "m",
            thickness: "cm",
            density: "t/m³",
            tonnage: "t",
            area: "m²"
        },
        step: "Schritt",
        of: "von",
        back: "Zurück",
        next: "Weiter",
        projectDetails: "Projekt",
        dimensions: "Maße",
        material: "Material",
        summary: "Vorschau",
        calculationsVerified: "Berechnung geprüft",
        trafficClass: "Belastungsklasse (RStO 12)",
        recommendedStructure: "Empfohlener Aufbau",
        thicknessWarning: "Hinweis: RStO-Empfehlung überschreibt aktuelle Werte",
        none: "Keine",
        privacyTitle: "Datenschutz im Fokus",
        privacyDescription: "Wir schätzen Ihre Privatsphäre. Wir verwenden Cookies, um Ihr Erlebnis zu verbessern und unseren Datenverkehr zu analysieren. In Übereinstimmung mit dem deutschen TDDDG erfassen wir anonyme Länderdaten nur nach Ihrer ausdrücklichen Zustimmung.",
        custom: "Anpassen",
        rejectAll: "Alle ablehnen",
        acceptAll: "Alle akzeptieren",
        settingsTitle: "Cookie-Einstellungen",
        essentialCookies: "Notwendige Cookies",
        essentialCookiesDesc: "Immer aktiv. Notwendig für die ordnungsgemäße Funktion der App.",
        analyticsCookies: "Analyse (Anonym)",
        analyticsCookiesDesc: "Hilft uns zu verstehen, aus welchen Ländern unsere Nutzer kommen.",
        apply: "Auswahl anwenden",
        downloadReport: "Einbauprotokoll herunterladen (PDF)",
        modeWorker: "Arbeiter-Modus",
        modeEngineer: "Ingenieur-Modus",
        modeWorkerDesc: "Schnelle Mengenberechnung",
        modeEngineerDesc: "RStO 12 & erweiterte Standards"
    }
};
